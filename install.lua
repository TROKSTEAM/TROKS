...
local RunBot = io.open("TROKS", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/TROKS
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr TROKS.lua
wget "https://raw.githubusercontent.com/TROKSTEAM/TROKS/main/TROKS.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./TROKS.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S TROKS -X kill
screen -S TROKS ./TROKS
done
]])
RunTs:close()
end
